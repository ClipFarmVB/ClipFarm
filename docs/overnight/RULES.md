...
- **A `Co-Authored-By` trailer and a session link on a COMMIT are fine** — the
  ...
  **The two named above are emitted client-side and are suppressible**, and a
  sandbox does not inherit the settings that suppress them — which is the
  mechanism behind the prohibition, and what the reversal nearly deleted along
  with the rest. Scoped to those two on purpose: a wider version of this
  sentence said every stamp on those four surfaces is run-added and removable,
  which is false and would have emptied the exemption below it.

  **It is false by measurement, not by argument.** On 2026-09-16 a run rewrote
  #554's body to end in a single footer, PATCHed it, and read it straight back;
  `_Generated by [Claude Code](https://claude.ai/code)_` came back on a body it
  had not been written into. So at least one footer on a PR body is neither
  run-added nor removable, and the exemption below is live rather than
  hypothetical.
...
**This section has broken the rule it states, while stating it** — in
`f025ce5`, `0a5e93c`, `1306251` and `2b9310f`, each fixing the instance before.
Those commit messages are the record; a squash carries them onto `main`. Keep
prose like this short: it is read on every lap, and every sentence is a place to
be wrong.
...
afterwards widened it into a family. That is a *sibling* of
[composing a claim before reading the result](#measure-what-you-publish) — there
the claim precedes the run, here it follows a run that was read correctly — and
no rule about *how* to run mutations reaches either.
