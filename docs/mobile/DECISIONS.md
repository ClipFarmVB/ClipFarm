...
**Resolving a conflict here.** This file lives on `main` so that branches writing
entries share a common ancestor for the path; before it did, git saw every pair
of mobile branches as an add/add conflict and offered a choice between two whole
files. **If you hit a conflict in this file, keep both sides** — never resolve it
by keeping one. Every entry the other branch carried is a decision that nothing
else in the repository records. See CF-386 (#503).

---

## Open — needs Owen's call

### CF-386 · Whether the log stays one file or becomes one file per entry — 2026-09-12

**The choice.** This file is a single document that entries are added to at the
top, which is the shape the epic's ticket procedure prescribes. CF-386 (#503)
proposes replacing it with `docs/mobile/decisions/CF-NNN-<slug>.md` — one file
per entry, with this file reduced to an index.

**Context.** Landing this file on `main` (the first *Decided* entry below) fixes
the expensive half of the problem: with no common ancestor for the path, git
reported an add/add conflict, and resolving that by taking one side silently
dropped every entry the other branch carried.

What it does not fix is the conflict itself. Once the branches have adopted the
ancestor, two of them adding entries both insert at the top of `## Decided` — the
same region of the same file — so git still stops and asks. It shows both sides
rather than offering a choice between two whole files, which is a large
improvement in the failure mode and no improvement at all in the frequency.

**Options.**

1. **Leave it as one file.** Conflicts stay routine but visible, and the
   resolution is mechanical: keep both entries, order by date. The reading order
   the format is built around — newest first, one scroll, one document you can
   hand someone — survives. The cost is a hand resolution on most mobile PRs, and
   the standing risk that somebody resolves one carelessly.

2. **One file per entry, this file as an index.** No two tickets ever write the
   same entry path, which is exactly the trick CF-315 (#365) uses to stop five
   screen tickets fighting over one route table. The conflict does not vanish,
   though — it moves to the index, where every ticket still appends a line. A
   one-line conflict is better than an entry-sized one, but "no more conflicts"
   would overstate it. The cost is the reading order: nine entries become nine
   files plus an index that can drift from them, and reviewing a decision
   alongside its PR means opening a second file.

3. **One file per entry, no index at all.** The directory listing is the index —
   filenames carry the CF number and slug, and Open-vs-Decided becomes a status
   line inside each file. The only option with genuinely no shared file and so
   genuinely no conflicts. It costs the most convenience: there is no single
   document any more, and "what is still open?" becomes a grep rather than a
   section.

**Recommended.** Option 1 for now — a recommendation to *not* spend the change.
The silent-loss risk was the part worth fixing and the entry below fixes it. What
remains is an ordinary merge conflict in a documentation file, which at nine
entries is cheaper to pay than to engineer around. Option 2 is the one to reach
for if mobile PR volume rises enough that the resolutions become real toil, and
it stays available: moving markdown is a `git mv` and a link fix, not a
migration.

The reason this is Owen's call rather than mine is narrower than the trade-off.
Options 2 and 3 change the file format the ticket procedure specifies, and
quietly reshaping the thing that procedure tells the next run to write is how a
convention stops being followed.

**Consequences.** Nothing blocks on this; every mobile ticket keeps writing this
file as it stands. If option 2 or 3 is chosen later the entries move wholesale,
and nothing that cites them by content breaks — only the links in this file and
any PR body that deep-linked a heading anchor.
...
### CF-386 · The log lands on `main`, and the union is the whole of it — 2026-09-12

**The choice.** This file is added to `main` on its own, carrying every entry
that existed on any in-flight mobile branch, before any of those branches merge.

**Context.** The log was introduced by the first mobile ticket that needed one
and then, independently, by the next three: `mobile/CF-318`, `mobile/CF-319`,
`mobile/CF-321` and `mobile/CF-322` each *added* `docs/mobile/DECISIONS.md` in
their own first commit. Git had no common ancestor for the path, so it treated
every pair as an add/add conflict — and the natural resolution, keeping one
side, discards every entry the other side wrote. Two of those branches already
carry a later commit titled *"carry the decision log forward instead of
restarting it"*, which is a run noticing by hand that its copy had gone stale.
CF-386 (#503) is the card.

The cost was never the conflict. It was that **merge order silently decided
which decisions survived**, in a file whose entire purpose is that the reasoning
is not lost.

**Options.** Land the file on `main` once so every branch shares an ancestor;
restructure it so no two tickets write the same path (the *Open* entry above,
now Owen's call); or keep resolving by hand, as the two carry-forward commits
already did.

**Chosen.** Land it on `main`, and take the union rather than adjudicating
between the copies.

The union turned out to need no adjudication at all, and that is the fact that
made this safe to do unattended. Extracting all four copies and comparing them
entry by entry: nine distinct entries exist across the four branches, every
entry that appears on more than one branch is **byte-identical** on each, and
the file header is byte-identical on all four. `mobile/CF-321-phone-rendition`
carries all nine — a strict superset. So the union *is* that copy, verbatim, and
no choice about anyone else's wording, ordering or precedence had to be made. Had
two copies genuinely disagreed on the text of a shared entry, that would have
been a question for whoever wrote it rather than something to settle in an
unattended run.

**Consequences.** Be precise about what this buys, because the obvious claim is
wrong. Landing the file does **not** by itself make the four in-flight merges
three-way. Every one of those branches forked from `6dd5e8b`, which is `main`'s
tip today, so this commit is *newer than their merge base* — git still sees the
path as added on both sides and still reports add/add until each branch merges
`main` in once. That was checked rather than assumed: synthesising this commit
on top of `main` and dry-running the merge still conflicts.

What it changes is the resolution, from an editorial judgement to a mechanical
one. `main`'s copy is a line-level superset of all four branch copies, so on
every one of them the correct resolution is **take `main`'s side wholesale** —
and that provably loses nothing, which was exactly the property the old add/add
did not have. Once each branch has done that, the path merges normally from then
on. Reconciling the four is the second half of CF-386; it is not done here
because it would mean pushing to four open PRs in a change whose whole value is
being small enough to check by eye.

So the conflict count is unchanged in the short run and the silent-loss risk is
gone. Those are different properties and only the second one was worth a PR.

The header now carries the convention, including that a conflict here is always
resolved by keeping both sides. It is stated in this file rather than in
`docs/overnight/TICKETS.md` because this is the file a run has open at the very
moment it is about to write an entry, which is where a convention actually gets
read.

**Worth knowing.** This is a general shape rather than a mobile one. Any file
that several branches are each expected to *create* — a changelog, a decision
log, an ADR index, a registry — has to exist on the mainline before the branches
that write it, or every merge after the first is a conflict with no base. Git's
three-way merge is only ever as good as the common ancestor it is given, and
with none it cannot tell an addition from a replacement, so it hands you both
files and no way to combine them. The fix is to land the seeded file first, which
is nearly free before the branches exist and costs a reconciliation per branch
afterwards.

---

### CF-321 · The phone rendition is transcoded from the cut clip, not cut again from the source — 2026-09-12

**The choice.** Each clip's 720p rendition is produced by re-encoding the clip
file that was just written, rather than by making a second scaled cut from the
game's source video.

**Context.** `ml/pipeline/clip.py` encodes every clip at source resolution, so a
4K upload yields 4K clips and a phone on cellular pays for pixels it cannot
resolve. Producing a reduced rendition means a second encode, and the only real
question is what that encode reads.

Two things constrain it. The source is up to 8 GB (`max_upload_bytes`), and
`generate_clips` already seeks into it once per detection. And CF-239 (#242) is
an open card that changes exactly this code path: it will stream-copy the cut
when the upload is already H.264/AAC and re-encode only when it is not, which
means that from CF-239 onward there is no single "the cut command" to hang a
second output off.

**Options.**

1. **A second cut from the source, with a scale filter.** Best quality — one
   generation of encoding instead of two. It seeks and decodes the multi-GB
   source a second time per clip, and it has to be written twice once CF-239
   lands, because the copy path and the encode path stop being the same command.

2. **One ffmpeg invocation with two outputs from one decode.** The efficient
   answer on paper: decode the segment once, write full-size and scaled. It is
   also the one that collides hardest with CF-239 — `-c copy` means there is no
   decoded frame to branch, so the two-output graph only exists on the re-encode
   path and CF-239 would have to special-case it.

3. **Transcode the finished clip.** Decodes seconds of video instead of seeking
   gigabytes, and reads the same input whichever way the clip was produced.
   Costs one extra generation of lossy encoding.

**Chosen.** Option 3. The deciding argument is the CF-239 interaction rather
than the cost: options 1 and 2 both make the rendition a branch of *how the clip
was cut*, so CF-239 would have to carry this feature through its change, and
whoever writes it inherits a merge they did not ask for. Option 3 depends only
on the clip existing, and CF-239 guarantees that on both of its paths — it
stream-copies only H.264/AAC sources, so the clip is browser-safe H.264 either
way and this reads it identically.

The generation loss is the real cost and it is small in this specific case: the
rendition is also being downscaled by a factor of 1.5–3, and downscaling hides
compression artefacts rather than compounding them. Paying it buys independence
between two cards that would otherwise have to land in order.
...
**Worth knowing.** The instinct from image pipelines is that you should always
derive a thumbnail from the original, never from another derivative, because
losses compound. For video that instinct is right about *quality* and often
wrong about *cost*: seeking into a large container is expensive and a second
decode of an already-short clip is cheap, so the derivative-of-a-derivative
route frequently wins on wall-clock by a wide margin. It is worth measuring
rather than assuming, which is why the numbers are on the card.

---

### CF-321 · The bound is on the short side, and "no rendition" is a normal answer — 2026-09-12

**The choice.** The rendition clamps the **short** side of the frame to 720px,
and a clip whose source is already at or under that bound gets **no rendition at
all** — `clips.mobile_url` stays NULL and the client falls back to `clip_url`.

**Context.** "720p" is nearly always implemented as "scale height to 720". That
is the same thing as the short side only for landscape footage. A game filmed in
portrait is 1080x1920, and clamping its *height* produces a 405x720 video — a
third of the intended width, on the orientation phones actually hold.

Rotation makes it worse. A phone recording portrait very often writes a
1920x1080 stream plus a rotate flag, and ffmpeg applies that flag when decoding.
So code that computes `1280x720` from probe output and passes it as an explicit
`scale=1280:720` is handing fixed dimensions to a frame that is actually
1080x1920 — which does not crop or letterbox, it *stretches*.

**Options.** *Clamp height and accept that portrait is wrong* — one line, and
wrong for exactly the users most likely to film on a phone. *Clamp the short
side using dimensions read from a probe* — correct until rotation metadata is
involved, at which point it is silently wrong in the stretch direction.
*Clamp the short side inside the filter expression, in ffmpeg's own `iw`/`ih`* —
slightly harder to read, and immune to rotation because those are evaluated
after the decoder has applied it.

**Chosen.** The third, with `min()` guards so an upscale is unrepresentable
rather than merely unlikely, and `-2` on the free axis so the aspect ratio
...
**Consequences.** This is the contract slice 2 hands CF-332 and CF-339:
`mobile_url` is `string | null` and the null branch is the common path, not an
edge case. Nothing may assume the column is populated. Because rotation is
handled by never naming explicit dimensions, there is also no probe-side
rotation parsing to keep correct — the skip decision compares `min(w, h)`, which
is the same number whichever way the probe reports the frame.

The renditions live under their own `clips-mobile/` prefix rather than a suffix
inside `clips/`, so the two can be told apart by prefix alone by anything that
sweeps or lifecycles storage. That mattered more than expected: the retention
sweep lists only `raw/`, so nothing ever comes back for a stray clip object, and
every deletion path had to learn the new key by hand (game delete, bulk clip
delete, the re-run purge in `sync_delete_game_clips`, and the
deleted-mid-processing purge). A missed one is a permanent orphan, not a delayed
one.

**Worth knowing.** Video dimensions are not one number pair, they are three that
disagree: the coded size, the display size after rotation metadata, and the
display size after the sample aspect ratio. Anything that computes a size in
your language and hands it to the encoder has picked one of the three and is
betting the others match. Expressing the constraint in the filter graph
(`iw`, `ih`, `min`, `-2`) lets the decoder resolve all three first, which is why
it is the form that survives real phone footage.
