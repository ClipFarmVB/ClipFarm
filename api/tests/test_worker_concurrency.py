...
def test_thread_limit_divides_cpus_across_children(monkeypatch):
    from app.workers import forksafe

    monkeypatch.setattr(forksafe.os, "cpu_count", lambda: 8)
...
def test_thread_limit_is_a_noop_at_concurrency_one(monkeypatch):
...
def test_explicit_thread_limit_wins(monkeypatch):
    from app.workers import forksafe
