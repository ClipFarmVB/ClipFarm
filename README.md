...
| `rate_limit_*_per_minute` | 30 / 60 / 120 | Per route group. The two enumerable routes (`/users/{handle}`, `/posts?username=`) key on the **client address even for a signed-in caller** — signup is self-serve, so a per-account bucket is one an attacker mints; the others key on the signed-in caller and fall back to the address. An address is a v4 host or a v6 /64. 30 for the handle-keyed routes (`/users/{handle}`, `/posts?username=`), which are the enumerable ones; 60 for `/games/{id}` and its clips, sized so the detail page's 5-second poll cannot throttle itself; 120 for `/clips/{id}/share` and `/posts/{id}`, where traffic is the success case. `/clips/{id}/download` is not in this table — it requires auth instead. |
