...
no app deps. v0 is the shipping rule-based path; the rest are the prototypes in
...
# 1s of wrongly cut play costs this many seconds of kept dead time. Same
# exchange rate the trainer uses (ml/eval/train_deadtime.LIVE_CUT_COST), so the
# net column here is comparable to its reports.
LIVE_CUT_COST = 4.0
...
def padding_ceiling(game: Game) -> DeadTimeSignals:
    """
    Most dead time the shipping pads can remove **without cutting any live
    play**: pretend detection is perfect — a contact every second of every
    ...
    Two things it does NOT bound, both of which show up in the table:
      ...
      - variants that change the pads. v4/v5 shrink them to 3/2 with merge 3
        (an 8s budget against the shipping 14s), which raises their own ceiling.
    It bounds the variants that keep the shipping pads: v0 through v3.
...
def render_game(game: Game, results: dict[str, tuple[list[Interval], DeadTimeSignals]]) -> str:
    ...
    <section>
      ...
      <p class="meta">{fmt_ts(duration)} ({duration:.0f}s) ·
        ...
        (perfect detection at the shipping pads, zero live cut — bounds
...
def render_summary(all_results: dict[str, dict[str, tuple[list[Interval], DeadTimeSignals]]]) -> str:
    ...
    <section>
      ...
      the same exchange rate <code>train_deadtime.py</code> uses. Higher is better;
      <code>v0</code> is what ships today.</p>
...
def build_html(games: list[Game], all_results: dict) -> str:
    parts = [
        ...
        "<code>v0</code> is the shipping path in <code>ml/pipeline/dead_time.py</code>; "
        f"<code>v1</code>–<code>{list(VARIANTS)[-1]}</code> are the prototypes in "
