...
- `video_duration_sec` — the full video length. **Required for correct numbers:**
  ...
  trailing dead time).
- `source_frame_height` — the labeled file's pixel height. Optional, but supply
...
config), and either a ball-cache entry for the video under the current
`TRACKING_CACHE_VERSION` or Modal to re-track it.
