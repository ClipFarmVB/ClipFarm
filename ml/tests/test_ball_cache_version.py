...
def test_the_track_shaping_inputs_have_not_moved_without_a_version_bump():
    ...
    assert RECORDED_FINGERPRINTS.get(version) == now, (
        ...
        "deploy, run `ml.eval.harness --offline` and `diagnose_detection` only "
        "with MODAL_TOKEN_ID/MODAL_TOKEN_SECRET unset: with Modal configured they "
        "key off the local `ml` but track on the deployed Modal app.\n"
