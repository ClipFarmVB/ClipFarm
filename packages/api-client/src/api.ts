...
import { getApiClientConfig, type ApiClientConfig } from "./config";

/**
 * The config is passed in rather than read here, so that a client nobody
 * configured fails loudly at the call site instead of being swallowed by the
 * catch below and going out as a signed-out request.
 */
async function getAuthHeaders(config: ApiClientConfig): Promise<Record<string, string>> {
  try {
    const token = await config.getToken();
    if (token) return { Authorization: `Bearer ${token}` };
  } catch {
    // Not logged in, or the token store is unreadable — fall through
  }
  return {};
}
